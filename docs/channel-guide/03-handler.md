...
Handler 是频道的入口点，负责工作流选择与执行。

## Handler 类定义

以下是 `apps/wechat/handler.py` 的核心实际代码：
...
Handler 通过 `get_planner_prompt()` 方法向 AI 规划器提供工作流信息：
...
```
execute_task_with_workflow(task)
            │
            ▼
    ┌───────────────┐
    │ is_complex_task? │
