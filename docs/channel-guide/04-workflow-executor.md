...
```
第1层：步骤级恢复
  └─ _try_recover() - 按返回键 + 导航回首页

第2层：导航级恢复
  └─ navigate_to_home_with_ai_fallback()
     ├─ 预定义方法（按返回键检测首页，最多5次）
     └─ AI辅助（分析界面给出导航建议，最多3次）

第3层：预置流程恢复
  └─ _ensure_at_home_screen()
     ├─ 并行检测多个目标（首页Tab、取消、返回）
     └─ 按优先级点击（首页 > 取消 > 返回 > 物理键）
```
