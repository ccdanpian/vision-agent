...
8. [预置流程 (Pre-Task Workflow)](#预置流程-pre-task-workflow)
9. [任务执行器 (TaskRunner)](#任务执行器-taskrunner)
10. [Fallback 机制详解](#fallback-机制详解)
11. [任务类型详细流程](#任务类型详细流程)
12. [错误处理与恢复](#错误处理与恢复)
13. [配置与环境](#配置与环境)
14. [测试指南](#测试指南)
...
│    - 支持变体: {name}_v1.png, {name}_v2.png ...             │
│    - 变体命名从 _v1 开始                                     │
└─────────────────────────────────────────────────────────────┘
    │
    ▼ (找到参考图)
...
│    - 抗旋转、缩放变换                                        │
│    - 适合复杂图标                                            │
│    - Lowe's ratio test 阈值: 0.9                            │
│    - 最终置信度阈值: 0.7 (匹配点数/总特征点)                 │
└─────────────────────────────────────────────────────────────┘
    │ 失败
    ▼
┌─────────────────────────────────────────────────────────────┐
...
## 预置流程 (Pre-Task Workflow)

在正式执行 AI 规划的任务步骤之前，系统会自动执行**预置流程**，确保应用处于正确的初始状态。

### 设计目标

- 确保目标应用已启动
- 确保应用在可操作的主页面
- 使用纯 OpenCV 检测，不调用 AI（快速、免费）
- 支持多变体参考图的并行检测
...
│ 等待 OPERATION_DELAY * 4                                     │
└─────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────┐
│ 步骤 1: 确保在消息首页 (循环检测，最多5次)                    │
├─────────────────────────────────────────────────────────────┤
│ 1. 截取屏幕                                                  │
│ 2. 并行检测多个目标（及其变体）:                              │
│    - wechat_home_button (+ _v1, _v2 变体)                   │
│    - wechat_cancel_button (+ 变体)                          │
│    - wechat_back (+ 变体)                                   │
│ 3. 根据检测结果决定行动:                                     │
│    - 检测到 home_button → 已在首页，流程结束 ✓              │
│    - 检测到 cancel_button → 点击取消按钮                    │
│    - 检测到 back_button → 点击返回按钮                      │
│    - 都未检测到 → 等待后重试                                │
│ 4. 等待 OPERATION_DELAY * 2 后继续下一次循环                │
└─────────────────────────────────────────────────────────────┘
    │
    ▼
进入正式任务执行
...
预置流程使用 `HybridLocator.locate_multiple_parallel()` 进行并行检测：

```python
def locate_multiple_parallel(
    self,
    screenshot: bytes,
    targets: Dict[str, list],  # {"target_name": [path1, path2, ...]}
) -> Dict[str, LocateResult]:
    """
    并行检测多个目标（仅 OpenCV，用于预置流程加速）

    特点:
    - 使用 ThreadPoolExecutor 并行执行所有检测
    - 每个目标检测其所有变体
    - 任一变体匹配成功即认为目标检测成功
    - 不使用 AI 回退，纯离线操作
    """
...
[预置] 等待应用启动... (1.2s)
[预置] 步骤1: 确保微信在消息页面
[预置] 尝试 1/5: 并行检测 3 个目标...
[预置]   - wechat_home_button: 未检测到
[预置]   - wechat_cancel_button: 检测到 (0.92)
[预置] 点击取消按钮...
[预置] 尝试 2/5: 并行检测 3 个目标...
[预置]   - wechat_home_button: 检测到 (0.95)
[预置] 确认: 已在微信消息首页
[预置] 预置流程完成，进入正式任务执行
```
...
当单个步骤执行失败时，系统会**自动等待后重试一次**，然后再考虑重新规划：
